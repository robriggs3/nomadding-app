var CityOps = (function () {
  ...
  var GENERATION_STAGES = [
    { id: 'eat', label: 'Eat and drink',
      sections: ['dinner', 'breakfast', 'lunch', 'coffee'] },
    { id: 'do', label: 'Things to do',
      sections: ['activities', 'cowork'] },
    { id: 'life', label: 'Services and practical',
      sections: ['services', 'practical'] }
  ];

  function generationStages() {
    // Copied out, so a caller cannot reorder the shipped list by accident.
    return GENERATION_STAGES.map(function (s) {
      return { id: s.id, label: s.label, sections: s.sections.slice() };
    });
  }
