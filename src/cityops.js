var CityOps = (function () {
  ...
  function emptyDeltaSummary() {
    // addedBySection lets a section-only paste say WHICH section it filled.
  ...
  function normalizeState(st) {
    ...
    // What each paste added, newest last, so "Undo last paste" can take back
    // exactly what it put in. Absent in every state written before this
    // shipped, so an old state simply reads as "no paste to undo".
    st.pasteLog = Array.isArray(st.pasteLog) ? st.pasteLog : [];
  ...
  // ---- Spreading undated picks across the stay (2026-09-06) ----
  //
  // PROMPT.md asks for one dinner per evening and one anchor activity per day,
  // dated. Generate used to deliver that and stopped: a run that has been
  // through a web-search tool loop drifts off the long parts of a schema
  // contract first, and dating every pick is one of those parts. The Plan tab
  // then opens empty on a freshly researched city, and the drag and day-move
  // controls have nothing to render on, so the whole planning surface reads as
  // broken.
  //
  // The fix lives here rather than in the prompt, because asking harder is the
  // approach we already have evidence against.
  //
  // FILL BLANKS ONLY (owner ruling 2026-09-06). An item that already carries a
  // day keeps it, whether the model dated it or the traveler did. A market that
  // only runs on Tuesday is exactly the pick a re-spread would ruin, and it is
  // also the pick a model is most likely to have got right.
  //
  // This proposes and does not mutate: it returns the assignments, and the
  // caller writes them through setDay like every other day change, so there is
  // one place that ever moves an item to a day.
  //
  // What it cannot do: group by geography. Items carry no coordinates (see
  // validateItem), so "put the two things near each other on the same day" is
  // not derivable from the data. What it does instead is keep the model's own
  // order, which is the closest thing to its intent that survives: it was told
  // to match each evening's dinner to that day's activity, and the order it
  // emitted them in is where that pairing lives.
  var SPREAD_KINDS = [
    // One per evening. Matched on id first, then on what the section calls
    // itself, so a guide that came back with "Evening meals" still spreads.
    { kind: 'dinner', perDay: 1, test: function (sec) {
      var id = String(sec && sec.id || '').toLowerCase();
      var label = String(sec && sec.label || '').toLowerCase();
      return id === 'dinner' || /dinner|evening meal|supper/.test(id + ' ' + label);
    } },
    // One anchor per day: the single trip into town the profile allows for.
    { kind: 'anchor', perDay: 1, test: function (sec) {
      return tabForSection(sec) === 'do';
    } }
  ];

  // data, state, opts {today} -> { assignments: [{id, day, kind}], days: [iso],
  //                               undatedLeft: n }
  // `today` trims days already gone: a traveler who lands on the fourth night
  // does not want three of their dinners assigned to dates that have passed.
  function spreadPlan(data, state, opts) {
    var o = opts || {};
    var st = state || emptyState();
    var range = effectiveDates(data, st);
    var days = stayDates(range);
    if (o.today) {
      var future = days.filter(function (iso) { return iso >= o.today; });
      // Only trim when something is left: a stay entirely in the past still
      // spreads across itself rather than returning nothing at all.
      if (future.length) days = future;
    }
    var secById = {};
    (data && Array.isArray(data.sections) ? data.sections : []).forEach(function (s) {
      if (s && s.id) secById[s.id] = s;
    });
    var items = (data && Array.isArray(data.items) ? data.items : []);

    // What each day already holds, per kind, so an existing assignment is
    // counted rather than doubled.
    var taken = {};
    SPREAD_KINDS.forEach(function (k) { taken[k.kind] = {}; });
    function kindOf(it) {
      var sec = secById[it.section];
      for (var i = 0; i < SPREAD_KINDS.length; i++) {
        if (SPREAD_KINDS[i].test(sec)) return SPREAD_KINDS[i];
      }
      return null;
    }
    items.forEach(function (it) {
      if (!it || !it.id) return;
      var day = effectiveDay(it, st);
      if (!day) return;
      var k = kindOf(it);
      if (!k) return;
      taken[k.kind][day] = (taken[k.kind][day] || 0) + 1;
    });

    var assignments = [];
    var undatedLeft = 0;
    SPREAD_KINDS.forEach(function (k) {
      // Order is the model's own, which is the only record of which dinner it
      // meant to sit beside which afternoon.
      var queue = items.filter(function (it) {
        if (!it || !it.id) return false;
        if (effectiveDay(it, st)) return false;              // fill blanks only
        if (effectiveStatus(it, st) !== 'plan') return false; // backups stay spare
        var mine = kindOf(it);
        return !!mine && mine.kind === k.kind;
      });
      var qi = 0;
      days.forEach(function (iso) {
        var already = taken[k.kind][iso] || 0;
        while (already < k.perDay && qi < queue.length) {
          assignments.push({ id: queue[qi].id, day: iso, kind: k.kind });
          qi++; already++;
        }
      });
      undatedLeft += Math.max(0, queue.length - qi);
    });
    return { assignments: assignments, days: days, undatedLeft: undatedLeft };
  }
  ...
  // ---- maps: where a place actually is ----
  //
  // Rob asked for this from Istanbul, standing in the city the guide describes.
  // A list of names is a reference; a map is a decision about where to walk
  // next, and that is the difference between the two.
  //
  // Colours follow the TABS, not a new vocabulary: a traveler has already
  // learned Eat & Drink, Do, Services and Info on every city page, so a marker
  // that matches its tab needs no legend to be read, only one to be confirmed.
  var MAP_CATEGORIES = [
    { id: 'eat', label: 'Eat and drink', color: '#c2410c' },
    { id: 'do', label: 'Things to do', color: '#0d6a72' },
    { id: 'services', label: 'Services', color: '#6d28d9' },
    { id: 'stay', label: 'Stay', color: '#b45309' },
    { id: 'other', label: 'Other', color: '#52525b' }
  ];

  // A place already done is context, not a candidate. It still earns a pin,
  // because "I ate there on Tuesday" is exactly what makes the pin next to it
  // interesting, but it stops competing for attention with what is still
  // ahead. Grey, and outside the category colours entirely, so a done dinner
  // never reads as a dinner still to book.
  var DONE_COLOR = '#9aa3a2';

  function mapCategories() {
    return MAP_CATEGORIES.map(function (c) { return { id: c.id, label: c.label, color: c.color }; });
  }

  // A section id (or a whole section object) to the bucket its marker is
  // coloured by. Built on tabForSection so the map and the tabs can never
  // disagree about what counts as eating.
  function mapCategory(sec) {
    var obj = (sec && typeof sec === 'object') ? sec : { id: String(sec || '') };
    var id = String(obj.id || '').toLowerCase();
    if (id === 'stay' || id === 'accommodation' || id === 'lodging' || id === 'hotel') return 'stay';
    var tab = tabForSection(obj);
    if (tab === 'eat') return 'eat';
    if (tab === 'do') return 'do';
    if (tab === 'services') return 'services';
    return 'other';
  }

  function mapColor(categoryId) {
    var hit = null;
    MAP_CATEGORIES.forEach(function (c) { if (c.id === categoryId) hit = c.color; });
    return hit || '#52525b';
  }

  // ---- reading coordinates out of a map link ----
  //
  // Every guide this app has ever produced carries a Google Maps link per
  // place, and most of them are a NAME SEARCH rather than a pin: measured on
  // Rob's Istanbul on 2026-09-17, 33 of 35 items had a map link and NONE of
  // them carried a latitude. So this parser is the cheap path, geocoding is
  // the fallback, and the honest expectation is that most cities start with
  // everything in the fallback.
  //
  // Returns {lat, lng} when the URL pins a point, or null. Never throws: it
  // runs over whatever a model wrote into a link field.
  var LATLNG = '(-?\\d{1,3}(?:\\.\\d+)?)\\s*,\\s*(-?\\d{1,3}(?:\\.\\d+)?)';

  function okLatLng(lat, lng) {
    if (!isFinite(lat) || !isFinite(lng)) return null;
    if (lat < -90 || lat > 90 || lng < -180 || lng > 180) return null;
    // 0,0 is the Atlantic. It is what a broken template writes, never a place
    // anybody planned to eat at.
  ...
  function parseLatLng(url) {
    ...
    var m;
    // geo:lat,lng
    m = new RegExp('^geo:' + LATLNG).exec(u);
    if (m) return okLatLng(parseFloat(m[1]), parseFloat(m[2]));
    // .../@lat,lng,17z  the shape a shared Google map uses
    m = new RegExp('@' + LATLNG).exec(u);
    if (m) return okLatLng(parseFloat(m[1]), parseFloat(m[2]));
    // !3dLAT!4dLNG inside a place URL
    m = /!3d(-?\d{1,3}(?:\.\d+)?)!4d(-?\d{1,3}(?:\.\d+)?)/.exec(u);
    if (m) return okLatLng(parseFloat(m[1]), parseFloat(m[2]));
    // ?q= / &query= / ?ll= / ?center= carrying a bare pair rather than a name
    m = new RegExp('[?&](?:q|query|ll|center|daddr|destination)=' + LATLNG + '(?:[&#]|$)', 'i').exec(u);
    if (m) return okLatLng(parseFloat(m[1]), parseFloat(m[2]));
    return null;
  }

  // What a link can still tell us when it has no coordinates: the place query
  // a human would have typed. Used to geocode, and better than the item name
  // alone because the model usually put the district in the link.
  function parseMapQuery(url) {
    var u = String(url === null || url === undefined ? '' : url);
    if (!u) return '';
    var m = /[?&](?:q|query)=([^&#]+)/i.exec(u);
    if (!m) m = /\/maps\/search\/([^/?#]+)/i.exec(u);
    if (!m) m = /\/maps\/place\/([^/@?#]+)/i.exec(u);
    if (!m) return '';
    var q = m[1];
    try { q = decodeURIComponent(q); } catch (e) { /* leave it as written */ }
    q = q.replace(/\+/g, ' ').replace(/\s+/g, ' ').replace(/^\s+|\s+$/g, '');
    // A cid or ftid link names nothing a geocoder can use.
    if (!q || /^\.{2,}$/.test(q) || q === '...') return '';
    return q;
  }

  // The first map link on an item, whatever else it carries.
  function itemMapLink(item) {
    var links = (item && Array.isArray(item.links)) ? item.links : [];
    var hit = '';
    links.forEach(function (l) {
      if (hit || !l || !l.href) return;
      if (l.kind === 'map' || /maps\.|\/maps\//.test(String(l.href))) hit = String(l.href);
    });
    return hit;
  }

  // Where one item is, and how we know. Order matters: a coordinate CACHED on
  // the item wins, because it was resolved once and must never be looked up
  // again; then the link; then nothing, and the item is listed as unplaced.
  function itemPoint(item) {
    if (!item) return null;
    var g = item.geo;
    if (g && typeof g === 'object') {
      var cached = okLatLng(Number(g.lat), Number(g.lng));
      if (cached) return { lat: cached.lat, lng: cached.lng, source: g.source || 'cache' };
    }
    var fromLink = parseLatLng(itemMapLink(item));
    if (fromLink) return { lat: fromLink.lat, lng: fromLink.lng, source: 'link' };
    return null;
  }

  // The query to geocode an unplaced item with. "name, city" is what Rob asked
  // for; the link's own query is used when it has one, because the model
  // usually wrote the district into it and a district is what separates two
  // cafes of the same name.
  // ---- how to ask, in order, and what to accept ----
  //
  // Measured against Rob's 33 Istanbul places on 2026-09-17, one strategy per
  // run, every result checked against Istanbul's own bounding box:
  //
  //   the link's query, street number and all   14 right,  0 wrong, 19 nothing
  //   "name, city"                              19 right,  2 WRONG, 11 nothing
  //   "name, district, city", district verified 19 right,  0 wrong, 14 nothing
  //
  // The middle row is the one to refuse. "Workinton, Istanbul" answers with the
  // Sisli branch and "Vodafone, Istanbul" with the Kagithane one: pins inside
  // the right city, on the wrong building, which is the failure a traveler
  // discovers by walking to it. A place missing from the map costs a glance at
  // the list underneath; a place in the wrong spot costs the evening.
  //
  // So: ask precisely first, then ask more loosely but VERIFY the district,
  // and never accept an answer that cannot be checked.
  function geocodePlan(item, cityData) {
    if (placeholderReason(item)) return [];
    var city = (cityData && cityData.city && cityData.city.name) ? String(cityData.city.name) : '';
    var link = parseMapQuery(itemMapLink(item));
    var name = (item && item.name) ? String(item.name) : '';
    var out = [];
    // 1. Exactly what the guide wrote. Precise, and on the measured run it was
    // right every single time it answered at all.
    var first = geocodeQuery(item, cityData);
    if (first) out.push({ q: first, district: '' });
    // 2. The same place without the street number, keeping the DISTRICT, which
    // is the token that tells two branches of a chain apart.
    var district = districtFrom(link, city, name);
    if (name && district) {
      // The NAME has to be shortened too, not just the query. Guides write
      // "Vodafone Store Muhurdar Caddesi" and "Ziraat Bankasi Kadikoy Branch
      // ATM" as the item name, so pasting it whole puts the street straight
      // back into the question the first attempt already failed on. Measured
      // 2026-09-17: the full name placed 15 of 33, the short name 19.
      out.push({ q: shortName(name, district, city) + ', ' + district +
        (city ? ', ' + city : ''), district: district });
    }
    return out;
  }

  // What a place is CALLED, with the address and the branch words taken off.
  // Keeps at least the first two words, because a one-word name is usually a
  // chain and a chain needs whatever else the guide gave us.
  var BRANCH_WORDS = /^(branch|subesi|sube|atm|store|magaza|magazasi|shop|outlet|location)$/i;

  function shortName(name, district, city) {
    var words = String(name || '').split(/\s+/);
    var dN = bareWord(district), cN = bareWord(city);
    var kept = [];
    var stop = false;
    words.forEach(function (w) {
      var b = bareWord(w);
      if (!b) return;
      // Everything from the first street word on is address, not name.
      if (isStreetWord(w)) { stop = true; return; }
      if (stop) return;
      if (b === dN || b === cN) return;
      if (BRANCH_WORDS.test(b)) return;
      kept.push(w.replace(/[,]+$/, ''));
    });
  ...
  // The district named in a link query: the last word-group that is not the
  // city, the place name, or a street. Turkish street words are listed because
  // this app's guides are full of them and "Recaizade Sokak" is not a district.
  // ANCHORED, and matched against the bare word. Unanchored, `st` matched
  // "Istanbul" and every district in the country was read as a street.
  var STREET_WORDS = /^(caddesi|cadde|cad|sokak|sokagi|sok|bulvari|bulvar|mahallesi|mahalle|mah|street|avenue|road|st|ave|rd)$/i;

  function isStreetWord(w) { return STREET_WORDS.test(bareWord(w)); }

  // Punctuation is stripped from BOTH sides before comparing, or a name
  // written "Adem Baba, Arnavutkoy" leaves the token "Baba," unmatched against
  // the name word "Baba" and the district comes back as the second half of the
  // restaurant's own name. Caught on Rob's guide, 2026-09-17.
  function bareWord(w) {
    return normLoose(String(w || '').replace(/[^0-9A-Za-z\u00C0-\u024F\u0130\u0131]+/g, ''));
  }

  // The district is the word immediately BEFORE the city, and nothing else.
  //
  // An earlier version scanned for "the last plausible token", which is the
  // kind of rule that works on the examples you wrote it against and then
  // reads "Kemankes" out of "Kemankes Caddesi" and "Baba" out of "Adem Baba".
  // Guides write "<place> <district> <city>", so that is the only position
  // worth trusting; when the word there is a street, a number, or part of the
  // name, there is no district and the second attempt is simply not made.
  function districtFrom(linkQuery, city, name) {
    var q = String(linkQuery || '');
    var cityN = bareWord(city);
    if (!q || !cityN) return '';
    var toks = q.replace(/,/g, ' ').split(/\s+/).filter(function (w) { return bareWord(w); });
    var at = -1;
    for (var k = toks.length - 1; k >= 0; k--) {
      if (bareWord(toks[k]) === cityN) { at = k; break; }
    }
    if (at <= 0) return '';
    var w = toks[at - 1];
    var n = bareWord(w);
    ...
    if (/\d/.test(w)) return '';
    if (isStreetWord(w)) return '';
    var nameWords = {};
    String(name || '').split(/\s+/).forEach(function (x) {
      var b = bareWord(x);
      if (b) nameWords[b] = 1;
    });
    if (nameWords[n]) return '';
    return w.replace(/[^0-9A-Za-z\u00C0-\u024F\u0130\u0131]+/g, '');
  }


  // Diacritic and Turkish-dotless-i folding, so "Kadikoy" matches "Kadıköy".
  // Nominatim answers in the local script; the guides are written both ways.
  function normLoose(sv) {
    return String(sv === null || sv === undefined ? '' : sv)
      .toLowerCase()
      .replace(/[\u0130\u0131]/g, 'i')
      .normalize ? String(sv === null || sv === undefined ? '' : sv).toLowerCase()
        .replace(/[\u0130\u0131]/g, 'i')
        .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      : String(sv || '').toLowerCase();
  }

  // Is this answer good enough to pin? Two gates, and both have to pass.
  //
  // `within` is the city's own bounding box, PADDED. A tight box rejects a
  // right answer: "Transfer to Istanbul Airport" resolves to Istanbul Airport
  // at 41.27, 28.73, which is genuinely Istanbul and sits outside the box
  // Nominatim gives for the city proper.
  var BBOX_PAD_DEG = 0.35;

  function acceptGeocode(res, opts) {
    var o = opts || {};
    if (!res) return false;
    var pt = okLatLng(Number(res.lat), Number(res.lng));
    if (!pt) return false;
    var bb = o.bbox;
    if (bb && bb.length === 4) {
      if (pt.lat < bb[0] - BBOX_PAD_DEG || pt.lat > bb[1] + BBOX_PAD_DEG) return false;
      if (pt.lng < bb[2] - BBOX_PAD_DEG || pt.lng > bb[3] + BBOX_PAD_DEG) return false;
    }
    // When the question named a district, the answer has to be in it. This is
    // the gate that keeps Workinton out of Sisli.
    if (o.district) {
      var hay = normLoose(res.display_name || res.name || '');
      if (hay.indexOf(normLoose(o.district)) === -1) return false;
    }
    return true;
  }

  function geocodeQuery(item, cityData) {
    // Never spend a lookup on the app's own template. Measured on Rob's
    // Istanbul: without this the queue would have asked Nominatim for
    // "Fill this city with real data, Istanbul" and pinned it somewhere.
    if (placeholderReason(item)) return '';
    var city = (cityData && cityData.city && cityData.city.name) ? String(cityData.city.name) : '';
    var fromLink = parseMapQuery(itemMapLink(item));
    var name = (item && item.name) ? String(item.name) : '';
    var base = fromLink || name;
    if (!base) return '';
    // Do not repeat the city when the link already names it.
    if (city && base.toLowerCase().indexOf(city.toLowerCase()) !== -1) return base;
    return city ? base + ', ' + city : base;
  }

  // Everything the map needs for a set of items: the ones it can draw, and the
  // ones it cannot, which the page lists underneath rather than hiding.
  // Session-only, and deliberately NOT persisted. A filter that quietly
  // survives a reload is how a traveler opens the app next week, finds half
  // his guide missing from the map, and concludes it lost his data. Hiding is
  // something you do for a minute while you look at something else; it resets.
  var hideDoneOnMaps = false;

  function hideDoneNow() { return hideDoneOnMaps; }

  function setHideDone(v) { hideDoneOnMaps = !!v; return hideDoneOnMaps; }

  function mapPoints(data, items, opts) {
    var o = opts || {};
    var list = Array.isArray(items) ? items : ((data && data.items) || []);
    var secById = {};
    ...
    list.forEach(function (it) {
      ...
      var p = itemPoint(it);
      var cat = mapCategory(secById[it.section] || { id: it.section });
      if (p) {
        var isDone = it.status === 'done';
        // Hidden means GONE from the map, which is what was asked for. Dimming
        // was already spent on Near, and two kinds of faint pin on one map is
        // no signal at all.
        if (isDone && o.hideDone) return;
        // NUMBERED to match the list beneath, in LIST order, not by distance:
        // the list is the thing the traveler arranges by hand, so "the third
        // one down" and "pin 3" have to be the same object.
        points.push({ id: it.id, n: points.length + 1,
          name: it.name || it.id, section: it.section,
          category: cat, color: isDone ? DONE_COLOR : mapColor(cat),
          done: isDone,
          lat: p.lat, lng: p.lng, source: p.source,
          byHand: isHandPlaced(it), day: it.day || null });
      } else {
        var why = placeholderReason(it) ? 'looks like example text, not a real place'
          : (geocodeQuery(it, data) ? 'not looked up yet' : 'no address or map link to go on');
        missing.push({ id: it.id, name: it.name || it.id, section: it.section,
          category: cat, why: why, lookupable: !!geocodeQuery(it, data),
          placeholder: !!placeholderReason(it) });
      }
    });
    return { points: points, missing: missing };
  }

  // Writes a resolved coordinate ONTO the item, so it rides through the normal
  // sync and is never looked up twice. Returns a new guide; the input is not
  // touched.
  function cacheGeo(data, id, lat, lng, source) {
    var ok = okLatLng(Number(lat), Number(lng));
    if (!data || !Array.isArray(data.items) || !ok) return null;
    var found = false;
    var out = deepClone(data);
    out.items.forEach(function (it) {
      if (!it || it.id !== id) return;
      found = true;
      it.geo = { lat: ok.lat, lng: ok.lng, source: source || 'nominatim',
  ...
  //
  // 20 of Rob's 35 Istanbul places are not in OpenStreetMap at all: 18 searched
  // and absent, 2 with no address to search on. No amount of query tuning finds
  // a cafe the map has never heard of, and the geocoder is right to refuse the
  // guesses it cannot verify. So the traveler places it himself. He can see the
  // city, he knows where the place is, and his own pin is better evidence than
  // anything this app can infer.
  //
  // A hand pin needs no protection from the geocoder: itemPoint reads item.geo
  // first, and mapPoints only reports an item as missing when itemPoint returns
  // nothing, so a placed item is never looked up again. It rides the normal
  // sync like any other field.
  //
  // NO BOUNDING BOX CHECK HERE, deliberately, and this is the one place that
  // rule differs from geocodePlan. The geocoder checks its answers against the
  // city box because it is GUESSING; the traveler is not. He is looking at the
  // map while he drags. Refusing his pin for sitting outside a padded box would
  // be the app telling him it knows his trip better than he does, and a day
  // trip out of the city is an ordinary thing to have in a guide.
  function placeByHand(data, id, lat, lng) {
    return cacheGeo(data, id, lat, lng, 'manual');
  }

  // True for a pin the traveler put there. The shell offers "Move on map"
  // rather than "Place on map" for these, and says whose pin it is.
  function isHandPlaced(item) {
    return !!(item && item.geo && item.geo.source === 'manual');
  }

  // Take the coordinate off an item: undo for a pin dropped in the wrong spot,
  // and the only way to send a wrongly cached geocode back for another look.
  // Returns a new guide, or null if there was nothing to clear, so the caller
  // can tell "undone" from "nothing to undo" without comparing guides.
  function clearPoint(data, id) {
    if (!data || !Array.isArray(data.items)) return null;
    var had = false;
    var out = deepClone(data);
    out.items.forEach(function (it) {
      if (!it || it.id !== id || !it.geo) return;
      had = true;
      delete it.geo;
    });
    return had ? out : null;
  ...
  // ---- "Near": what else is worth doing while I am over there ----
  //
  // Rob's words: "This helps me arrange and align activities that are near each
  // other." So this is not a radius search, it is PAIRING. He is holding one
  // place and wants to know what else in the whole guide is a walk away, which
  // is why it searches ACROSS TABS: the point is pairing a dinner with an
  // activity, and those live on different tabs.

  // 1.2 km, quoted to the traveler as "about 15 minutes". Beyond it nothing is
  // listed at all: there is no "near-ish", because a list that trails off into
  // half-hour walks is a list nobody trusts the top of.
  var NEAR_RADIUS_M = 1200;
  // 80 metres a minute, 4.8 km/h. An unhurried walk with a bag, not a stride.
  var WALK_M_PER_MIN = 80;

  function toRad(d) { return d * Math.PI / 180; }

  // Straight-line metres. Deliberately NOT a routing service: this runs with no
  // network, instantly, on a cached coordinate the guide already carries, and
  // it is right for the question being asked. A river or a hill can make a
  // 900 m line a 20 minute walk, which is why the copy says "about".
  ...
  // gives; next door is "about 5 minutes" and that is honest at this precision.
  function walkMinutes(metres) {
    if (metres === null || metres === undefined || !isFinite(metres)) return null;
    var m = Math.round(metres / WALK_M_PER_MIN / 5) * 5;
    return m < 5 ? 5 : m;
  }

  // Everything in the guide within a walk of one item, nearest first.
  // Excludes itself, archived items and placeholder text, and anything not
  // placed: an unplaced item has no coordinate to compare, and guessing at one
  // is the wrong-pin failure all over again.
  function nearbyOf(data, id, opts) {
    var o = opts || {};
    var radius = o.radiusM || NEAR_RADIUS_M;
    var items = (data && Array.isArray(data.items)) ? data.items : [];
    var self = null;
    items.forEach(function (it) { if (it && it.id === id) self = it; });
    var from = itemPoint(self);
    if (!from) return [];
    var secById = {};
    ((data && data.sections) || []).forEach(function (sec) { if (sec && sec.id) secById[sec.id] = sec; });
    var out = [];
    items.forEach(function (it) {
      if (!it || it.id === id || it.status === 'archived') return;
      if (placeholderReason(it)) return;
      var p = itemPoint(it);
      if (!p) return;
      var m = haversineM(from, p);
      if (m === null || m > radius) return;
      var sec = secById[it.section] || { id: it.section };
      var cat = mapCategory(sec);
      out.push({ id: it.id, name: it.name || it.id, section: it.section,
        sectionLabel: sec.label || '', tab: tabForSection(sec),
        tabLabel: tabLabel(tabForSection(sec)),
        category: cat, color: mapColor(cat),
        metres: m, minutes: walkMinutes(m) });
    });
    // Nearest first, then by name so the order is stable between renders for
    // two places the same distance away.
  ...
  function tabLabel(tabId) {
    var found = '';
    TABS.forEach(function (t) { if (t.id === tabId) found = t.label; });
    return found;
  }

  // The one line above the list. Says the radius in the unit a person walks in.
  function nearSummaryText(list, name) {
    var who = name ? ('of ' + name) : 'of here';
    if (!list || !list.length) {
      return 'Nothing else in this guide is within a 15 minute walk ' + who + '.';
    }
    return (list.length === 1 ? '1 place is' : list.length + ' places are') +
      ' within about a 15 minute walk ' + who + '.';
  }

  // Why a card has no Near control, when it has none. An unplaced card gets
  // the "Place it" answer rather than an empty list, and since #51 that is
  // something the traveler can actually do in one tap.
  function nearUnavailableText(item) {
    if (!item) return '';
    if (placeholderReason(item)) return '';
    if (itemPoint(item)) return '';
    return 'Not on the map yet, so there is nothing to measure from. ' +
      'Place it and Near starts working for it.';
  }
  ...
  // maxTokens is the budget the ANSWER has to fit in. Without it this pass asks
  // for the whole stay's coverage in one call and is cut off mid-JSON:
  // measured live 2026-09-17 on a 10-day Istanbul, it asked for 54 items,
  // returned exactly 12,000 output tokens (the ceiling) and 27,845 characters
  // with five unclosed brackets, after 139 seconds. One answer holds about 34
  // items at 350 tokens each, so it was over budget by twenty before it began.
  function buildResearchAllPrompt(templateText, cityData, profile, maxTokens) {
    ...
    if (!profileIsEmpty(p)) out = out.concat(interestsBlockLines(p, null));
    out.push('## What I need', '', rerun, '');
    // The counts, computed from the stay, in place of the fixed total the
    // template used to carry. Without this a nine-night city gets the same
    // handful as a three-night one, which is exactly what happened to Rob.
    out.push(coverageBlock(cityData, null, maxTokens || null), '');
    out.push('## Existing items (do not re-suggest these)', '');
    out = out.concat(itemListLines(cityData && cityData.items));
    out.push('');
    out.push('## Item shape', '', contract, '');
  ...
  // ---- staged generation: a whole city, a few short passes at a time ----
  //
  // A whole-city Generate used to be ONE call asking for everything, with
  // max_tokens 32000 and adaptive thinking. On 2026-09-09 that call ran for
  // 400.2 seconds through the managed proxy and emitted no output token at
  // all: the Supabase edge function's paid wall clock killed the isolate while
  // the model was still thinking. HTTP 200, 1,118 bytes, thirteen keepalive
  // pings, nothing else, and a traveler with nothing to show for four minutes.
  //
  // The fix is not a bigger ceiling, it is a smaller unit of work. Each stage
  // below is the SAME SHAPE as the research pass that already works, which
  // returned a complete delta in 13.9 seconds on the same key the same
  // morning. mergeDelta already merges those deltas and already refuses
  // duplicate ids, so nothing new had to be invented to hold them together.
  //
  // Three properties are the point, and each is worth more than the speed:
  //   - a stage that lands is KEPT. A failure in the last stage used to leave
  //     nothing; now it leaves a city with its eat and drink and its things to
  //     do, and a retry that resumes rather than restarts.
  //   - every stage is far inside the wall clock, so the ceiling stops being
  //     something the traveler can hit at all.
  //   - the traveler is told which stage is running, because four silent
  //     minutes is what made this feel broken even on the days it worked.
  // ---- how many of each thing a stay actually needs ----
  //
  // Rob's 9-day city came back with THREE places to eat. Nothing was broken:
  // PROMPT.md asked for "10 to 18 new items in total, spread across the
  // categories", and the eat stage then split its share four ways across
  // dinner, breakfast, lunch and coffee. A fixed total cannot serve a 3-night
  // stay and a 9-night stay, and three dinners for nine evenings is that
  // arithmetic working exactly as written.
  //
  // So the counts are computed from the stay and PRINTED in the prompt. The
  // split is between things a traveler chooses per day and things they look up
  // once: dinner and activities scale with the nights, because there is one
  // evening and one anchor to fill per day; a SIM shop, a laundry and an
  // airport run are facts about the CITY, and padding them with the stay
  // length would pad the guide rather than fill it.
  var COVERAGE_FIXED = { cowork: 2, services: 5, practical: 6 };

  function stayLengthDays(cityData) {
    var dates = cityData && cityData.city && cityData.city.dates;
    if (!dates || !dates.from || !dates.to) return 1;
    var n = stayDates(dates).length;
    return n > 0 ? n : 1;
  }

  // Returns {days, sections: {id: {picks, backups, total}}, total}.
  function coverageTargets(cityData) {
    var days = stayLengthDays(cityData);
    var daytime = Math.max(3, Math.ceil(days / 3));
    var sections = {
      // One dated pick per evening, plus somewhere to fall back to when a
      // place is shut, full, or simply wrong on the night.
      dinner: { picks: days, backups: Math.max(5, Math.ceil(days / 2)) },
      breakfast: { picks: daytime, backups: 0 },
      lunch: { picks: daytime, backups: 0 },
      coffee: { picks: daytime, backups: 0 },
      // One anchor per day, and four spares for weather and closures.
      activities: { picks: days, backups: 4 }
    };
    Object.keys(COVERAGE_FIXED).forEach(function (id) {
      sections[id] = { picks: COVERAGE_FIXED[id], backups: 0 };
    });
    var total = 0;
    Object.keys(sections).forEach(function (id) {
      sections[id].total = sections[id].picks + sections[id].backups;
      total += sections[id].total;
    });
    return { days: days, sections: sections, total: total };
  }

  // What one item costs to write. This was 350, taken from three stages on
  // 2026-09-14, and it was UNDER the true mean, so every budget built on it has
  // been optimistic since.
  //
  // Across sixteen live stages to 2026-09-17: 81,189 output tokens for 176
  // items. Mean 461. Worst single stage 558.
  //
  // A budget constant should sit on the WORST case, not the average, because
  // the cost of being wrong is not a slightly long answer, it is an answer cut
  // off mid-JSON that the traveler cannot use at all. Measured live on Rob's
  // 10-day Istanbul: at 350 the Research pass asked for 54 items, then for 30
  // after scaling, and BOTH runs hit the 12,000 ceiling exactly and truncated.
  // 560 budgets the same answer for 21 items, which fits with room to spare.
  //
  // Generation stages are unaffected: they ask for 8 to 17 items, all well
  // inside 21. This only bites the whole-guide Research pass, which is the one
  // that was over-asking.
  var TOKENS_PER_ITEM = 560;

  // The stage list for a given stay. A longer stay does not just ask for more,
  // it asks in more sittings, because one stage has to fit in one answer.
  //
  // days > 4  : eat splits into dinner and daytime
  // days > 6  : do splits into activities and cowork+practical, and services
  //             stands on its own
  // Every core section appears exactly once at every length, which is what the
  // coverage test pins: a section owned by no stage is never generated, and
  // nothing says so.
  function generationStages(cityData) {
    var days = cityData ? stayLengthDays(cityData) : 1;
    if (days > 6) {
      return [
        { id: 'dinner', label: 'Dinner', sections: ['dinner'] },
        { id: 'daytime', label: 'Breakfast, lunch and coffee',
          sections: ['breakfast', 'lunch', 'coffee'] },
        { id: 'activities', label: 'Things to do', sections: ['activities'] },
        { id: 'work', label: 'Cowork and practical', sections: ['cowork', 'practical'] },
    ...
    if (days > 4) {
      return [
        ...
        { id: 'daytime', label: 'Breakfast, lunch and coffee',
          sections: ['breakfast', 'lunch', 'coffee'] },
        { id: 'do', label: 'Things to do', sections: ['activities', 'cowork'] },
        { id: 'life', label: 'Services and practical', sections: ['services', 'practical'] }
      ];
    }
    return [
      { id: 'eat', label: 'Eat and drink', sections: ['dinner', 'breakfast', 'lunch', 'coffee'] },
      { id: 'do', label: 'Things to do', sections: ['activities', 'cowork'] },
      { id: 'life', label: 'Services and practical', sections: ['services', 'practical'] }
    ];
  }

  // Per stage. This was 7000 for one release, chosen to sit under ai_reserve's
  // large-call threshold of 8000, and that was a mistake worth leaving written
  // down: it sized the answer to suit the METER instead of the work, and the
  // work did not fit.
  //
  // Measured against the live proxy on 2026-09-14, one stage, same prompt:
  //   7000, thinking adaptive   -> out 7000 (the ceiling), NO text at all,
  //                               the whole budget spent on thinking
  //   7000, thinking disabled   -> out 7000 (the ceiling), text truncated
  //                               mid-structure, nothing usable
  //   12000, thinking disabled  -> out 6814, finished on its own, 15 items
  // A stage needs a shade under 7000 tokens to say what it has to say, so a
  // 7000 ceiling cut it off every single time.
  //
  // 12000 is over the large-call threshold, so each stage IS metered as a
  // large call and one guide spends three of the five an hour. That is the
  // honest trade: a guide a traveler can actually generate, with room for one
  // retry in the same hour, beats three cheap calls that always truncate.
  var STAGE_MAX_TOKENS = 12000;

  // Stages do NOT think.
  //
  // With adaptive thinking on, two of three stages spent the ENTIRE output
  // budget reasoning and emitted no text whatsoever (2026-09-14). A stage is
  // not a reasoning problem: it is a contract, a list of sections, and a list
  // of places not to repeat, all spelled out in the prompt. Thinking here buys
  // nothing and competes with the answer for the same tokens.
  var STAGE_THINKING = { type: 'disabled' };

  // The counts a prompt prints, for one set of sections, already scaled to fit
  // whatever budget the caller has. Returns {lines, items, scaled}.
  //
  // Scaling down rather than truncating is the whole point. A stage that asks
  // for more than it can say runs out mid-JSON and the traveler gets nothing,
  // which is exactly how the 7000-token release failed. A stage that asks for
  // fewer, honestly, returns a shorter guide that actually parses.
  function coverageLines(cityData, sectionIds, maxTokens) {
    var t = coverageTargets(cityData);
    var ids = (sectionIds && sectionIds.length) ? sectionIds : Object.keys(t.sections);
    var wanted = 0;
    ids.forEach(function (id) { if (t.sections[id]) wanted += t.sections[id].total; });
    var room = maxTokens ? Math.floor(maxTokens / TOKENS_PER_ITEM) : wanted;
    var scale = (wanted > room && wanted > 0) ? (room / wanted) : 1;
    var lines = [];
    var items = 0;
    ids.forEach(function (id) {
      var c = t.sections[id];
      if (!c) return;
      var picks = Math.max(1, Math.floor(c.picks * scale));
    ...
    return { lines: lines, items: items, scaled: scale < 1, days: t.days };
  }

  // The block every generated prompt carries instead of a fixed total.
  function coverageBlock(cityData, sectionIds, maxTokens) {
    var c = coverageLines(cityData, sectionIds, maxTokens);
    var out = ['## How many', '',
      'This stay is ' + c.days + (c.days === 1 ? ' day' : ' days') + ' long, so it needs:', ''];
    out = out.concat(c.lines);
    out.push('');
    out.push('Those numbers are the target, not a ceiling to stop short of. ' +
      'Return ' + c.items + ' items in total for the sections listed. A guide with ' +
      'fewer dinners than evenings is the one failure a traveler notices on day one.');
    if (c.scaled) {
      out.push('');
      out.push('(These counts are trimmed to what one answer can hold. Run the pass ' +
        'again afterwards for more: the guide lists what it already has, so a ' +
        'second pass adds rather than repeats.)');
    }
    return out.join('\n');
  }

  // One stage's prompt: the research pass, narrowed to this stage's sections.
  //
  // It reuses buildResearchAllPrompt wholesale rather than growing a second
  // prompt to keep in step with it. That matters because the research pass
  // already lists the city's existing items as "do not re-suggest these", so
  // stage two automatically knows what stage one produced, with no bookkeeping
  // here and no way for the two to disagree.
  function buildStagePrompt(templateText, cityData, profile, stage) {
    var s = stage || {};
    var ids = Array.isArray(s.sections) ? s.sections : [];
    if (!ids.length) throw new Error('a generation stage must name its sections');
    var base = buildResearchAllPrompt(templateText, cityData, profile);
    var narrowing = [
      '',
      coverageBlock(cityData, ids, STAGE_MAX_TOKENS),
      '',
      '## This pass covers ONE part of the guide',
      '',
      'Return items for these sections ONLY: ' + ids.join(', ') + '.',
      'Other sections are being filled by their own passes. An item for any ' +
        'section not in that list will be discarded, so spending output on one ' +
        'wastes the run.',
      'Cover the listed sections properly rather than sampling them: this is ' +
  ...
  function coverageShortfall(cityData, delta, sectionIds) {
    ...
    var ids = (sectionIds && sectionIds.length) ? sectionIds : Object.keys(t.sections);
    var got = {};
    // COUNT THE GUIDE AS IT WILL BE, not the delta on its own.
    //
    // Found on the live site 2026-09-16: pasting two extra dinners into a guide
    // that already held fourteen said "Dinner came up short: 2 places for 9
    // evenings. The answer was probably cut off." Both halves were false. A
    // delta is a TOP-UP, and measuring a top-up against the whole stay makes
    // every normal paste look like a failure; the "Let Nomadding detect" case
    // went further and reported seven sections short on a full guide.
    //
    // The question the warning exists to answer is "does this guide still have
    // fewer dinners than evenings", and that can only be asked of the merged
    // total. An empty city that gets a short paste still warns, which is the
    // case it was written for.
    var existing = (cityData && Array.isArray(cityData.items)) ? cityData.items : [];
    existing.concat((delta && Array.isArray(delta.items)) ? delta.items : [])
      .forEach(function (it) {
  ...
  //
  // The stage prompt tells the model that an item outside its sections will be
  // discarded. If the code did not actually discard it, that sentence would be
  // a bluff, and worse, a stray item would land in a section a LATER stage is
  // about to fill properly, where it would then be listed as "do not
  // re-suggest" and quietly crowd out the real pass.
  //
  // Returns a new delta; the input is never touched. A delta carrying no items
  // comes back unchanged, because ratings and intel blocks are keyed by item id
  // and belong to whatever item already exists, not to this stage.
  function keepStageItems(delta, stage) {
    var d = delta || {};
    if (!Array.isArray(d.items)) return d;
    var ids = (stage && Array.isArray(stage.sections)) ? stage.sections : [];
    var out = {};
    Object.keys(d).forEach(function (k) { out[k] = d[k]; });
    out.items = d.items.filter(function (it) {
      return it && ids.indexOf(it.section) !== -1;
    });
    return out;
  }
  ...
  // the errors they get are about the box they wanted.
  function looksWholeGuide(d) {
  ...
  // ---- when the words came from an IN-APP run, not a paste ----
  //
  // Rob, mid-trip 2026-09-17: a managed Research run sat at "Writing... 0
  // characters" for minutes, produced a fragment with three unclosed brackets,
  // and the app told him to "copy the file again and check you have all of it,
  // a preview pane often renders only the first screenful". He had not pasted
  // anything. The advice was for a paste, the failure was ours, and it read as
  // blame pointed at the one person who did nothing wrong.
  //
  // Takes the message intake produced and swaps the tail for what is actually
  // true of a run that stopped: nothing the traveler did caused it, nothing was
  // charged, and the way out is one button.
  function runFragmentText(intakeMessage) {
    var head = String(intakeMessage || '');
    // Everything up to the first sentence about copying is the DIAGNOSIS, and
    // that part is still true: the reply really did stop early.
    var cut = head.indexOf('. Nothing was applied');
    var what = cut === -1 ? head : head.slice(0, cut);
    what = what.replace(/^That paste /, 'That answer ').replace(/^That paste$/, 'That answer');
    return what + '. That is our run stopping early, not anything you did, and ' +
      'nothing was added to your guide. Press Run again. If it stops again, use ' +
      'Copy prompt and run it in your own Claude or ChatGPT, which has no time limit.';
  }

  // How long a delta pass is given before the app stops waiting. A stage-sized
  // pass answered in 28 to 90 seconds across every measured run on 2026-09-16,
  // so four minutes is generous rather than tight, and it is comfortably under
  // the 400 second edge-function ceiling that used to kill these runs silently.
  var AI_RUN_TIMEOUT_MS = 240000;

  function aiRunTimeoutText(ms) {
    var secs = Math.round((ms || AI_RUN_TIMEOUT_MS) / 1000);
    return 'Our server stopped answering after ' + secs + ' seconds, so this run ' +
      'was given up on. Nothing was added to your guide and nothing was charged ' +
      'to your allowance. Press Run again.';
  }

  // The progress line. "Writing... 0 characters so far" for four minutes is the
  // line Rob watched, and it says nothing a person can act on: not how long it
  // has been, not whether anything is happening, not what it is even doing.
  function aiRunProgressText(p) {
    var o = p || {};
    var secs = Math.max(0, Math.round((o.elapsedMs || 0) / 1000));
    var chars = o.chars || 0;
    var what = o.label ? o.label : 'Claude';
    var clock = secs + 's';
    if (!chars) {
      // The honest version of the old "0 characters": say it is thinking, and
      // say the wait is expected, because the silence IS the normal first phase.
      // Past ninety seconds that reassurance has expired, and repeating it is
      // its own small lie: a run watched to 239 seconds was still being told
      // the first words usually arrive within a minute.
      if (secs > 90) {
        return what + ': still thinking, ' + clock + ' so far. This is longer ' +
          'than usual. It will give up at ' + Math.round(AI_RUN_TIMEOUT_MS / 1000) +
          's and offer you a retry.';
      }
      return what + ': thinking, ' + clock + ' so far. The first words usually ' +
        'arrive within a minute.';
    }
    return what + ': writing, ' + chars.toLocaleString() + ' characters in ' + clock + '.';
  }

  // ---- taking back a paste, and spotting what should never have landed ----
  //
  // Rob, mid-trip 2026-09-17: the in-app run was broken so he pasted from chat,
  // and the chat had copied the TEMPLATE example out of the prompt and returned
  // it as a real place. His live Istanbul guide now holds "Real Place Name",
  // note "Why it fits, with rating and review count if you have it.", sitting
  // among twenty real Istanbul picks.
  //
  // Two things follow. The app should RECOGNISE that, because it is the app's
  // own template coming back, and nothing else looks like it. And a paste
  // should be undoable, because a traveler who let something in has no way to
  // pick it back out of a guide by hand.

  // The ids and phrasings the app itself authors. A place matching one of these
  // came from a template, not from a city.
  var PLACEHOLDER_IDS = { 'new-slug': 1, 'getting-started': 1 };
  var PLACEHOLDER_TEXT = [
    'Why it fits, with rating and review count if you have it.',
    'This is a blank scaffold.',
    'Real Place Name'
  ];

  // Returns [{id, name, section, why}] for items that are template leftovers.
  // It never removes anything: what to do about them is the traveler's call.
  // Why this ONE item looks like template text, or '' when it reads as real.
  // Shared, because the geocoder needs the same answer the cleanup offer does:
  // there is no point spending a lookup on "Real Place Name".
  function placeholderReason(it) {
    if (!it || !it.id) return '';
    if (PLACEHOLDER_IDS[it.id] === 1) {
      return 'the id `' + it.id + '` is one this app writes into its own template';
    }
    var hay = String(it.name || '') + ' ' + String(it.note || '');
    for (var i = 0; i < PLACEHOLDER_TEXT.length; i++) {
      if (hay.indexOf(PLACEHOLDER_TEXT[i]) !== -1) {
        return 'it still carries the template wording "' + PLACEHOLDER_TEXT[i] + '"';
      }
    }
    return '';
  }

  function placeholderItems(data) {
    var items = (data && Array.isArray(data.items)) ? data.items : [];
    var out = [];
    items.forEach(function (it) {
      var why = placeholderReason(it);
      if (why) out.push({ id: it.id, name: it.name || it.id, section: it.section || '', why: why });
    });
    return out;
  }

  function placeholderText(found) {
    if (!found || !found.length) return '';
    var named = found.map(function (f) {
      return '"' + f.name + '"' + (f.section ? ' in ' + f.section : '');
    }).join(' and ');
    return found.length + (found.length === 1 ? ' place looks like' : ' places look like') +
      ' example text rather than somewhere real: ' + named + '. That is this app\'s own ' +
      'template coming back in an answer, not somewhere you chose. Remove ' +
      (found.length === 1 ? 'it' : 'them') + '?';
  }

  // Removes items by id. Returns {data, removed}; the input is never touched,
  // and an id the guide does not have is simply not found.
  function removeItemsById(data, ids) {
    if (!data || !Array.isArray(data.items)) return { data: null, removed: 0 };
    var kill = {};
    (ids || []).forEach(function (id) { if (id) kill[id] = 1; });
    var out = deepClone(data);
    var before = out.items.length;
    out.items = out.items.filter(function (it) {
      return !(it && kill[it.id] === 1);
    });
    return { data: out, removed: before - out.items.length };
  }

  // The newest paste that still has something to take back.
  function lastPaste(state) {
    var log = (state && Array.isArray(state.pasteLog)) ? state.pasteLog : [];
    for (var i = log.length - 1; i >= 0; i--) {
      if (log[i] && Array.isArray(log[i].ids) && log[i].ids.length) return log[i];
    }
    return null;
  }

  // Newest last, and bounded: this rides along in the state blob that syncs
  // between devices, so it is a short memory rather than a history.
  var PASTE_LOG_MAX = 10;
  function recordPaste(state, entry) {
    var st = state || {};
    var log = Array.isArray(st.pasteLog) ? st.pasteLog.slice() : [];
    if (entry && Array.isArray(entry.ids) && entry.ids.length) {
      log.push({ at: entry.at || new Date().toISOString(),
        scope: entry.scope || '', ids: entry.ids.slice() });
    }
    while (log.length > PASTE_LOG_MAX) log.shift();
    st.pasteLog = log;
    return st;
  }

  // ---- Snapshots, so a wrong replace is recoverable by design ----
  //
  // The Batumi incident: a destructive write replaced a guide and the only
  // reason the traveler got it back was that a copy happened to exist
  // somewhere else. That is luck, and luck is not a feature. Every write that
  // can REMOVE work now keeps the previous version first.
  //
  // Three per city, newest last. Not a history: enough to undo the mistake you
  // just made and the one before it, and no more, because these hold whole
  // guides and live in the browser.
  //
  // THESE DO NOT SYNC, and that is deliberate. pasteLog rides in the state
  // blob because it is a list of ids; a snapshot is an entire guide, and
  // pushing three of them per city into the state row would multiply every
  // sync payload by four to protect against a mistake that is local anyway.
  var SNAPSHOT_MAX = 3;

  function addSnapshot(list, data, reason) {
    var out = Array.isArray(list) ? list.slice() : [];
    if (!data || !Array.isArray(data.items)) return out;
    out.push({ at: new Date().toISOString(), reason: String(reason || 'replace'),
      items: data.items.length, data: deepClone(data) });
    while (out.length > SNAPSHOT_MAX) out.shift();
    return out;
  }

  function newestSnapshot(list) {
    var l = Array.isArray(list) ? list : [];
    for (var i = l.length - 1; i >= 0; i--) {
      if (l[i] && l[i].data && Array.isArray(l[i].data.items)) return l[i];
    }
    return null;
  }

  // What a write actually did to a guide, counted by id rather than by length,
  // so a write that drops five places and adds five is not reported as no
  // change at all.
  function replaceDamage(before, after) {
    var b = (before && Array.isArray(before.items)) ? before.items : [];
    var a = (after && Array.isArray(after.items)) ? after.items : [];
    var had = {};
    b.forEach(function (it) { if (it && it.id) had[it.id] = 1; });
    var kept = 0, added = 0;
    a.forEach(function (it) {
      if (!it || !it.id) return;
      if (had[it.id]) { kept++; delete had[it.id]; } else added++;
    });
    var removed = 0;
    Object.keys(had).forEach(function () { removed++; });
    return { before: b.length, after: a.length, removed: removed, added: added, kept: kept };
  }

  // Worth interrupting the traveler for, or not.
  //
  // A silent snapshot covers everything; the BAR is for the writes somebody
  // would want to catch in the moment. Losing one place is an edit. Losing a
  // third of a guide, or more than three places, is an accident, and a
  // traveler mid-trip needs to see it while the undo is still one tap away.
  function isBigReplace(before, after) {
    var d = replaceDamage(before, after);
    if (d.removed === 0) return false;
    if (d.removed > 3) return true;
    return d.before > 0 && (d.removed / d.before) > 0.25;
  }

  // The bar's words. Says what was lost and where it came from, in that order,
  // because "34 places became 6" is the part that makes somebody act.
  function replaceUndoText(before, after, source) {
    var d = replaceDamage(before, after);
    var from = source ? (' from ' + source) : '';
    if (d.added > 0) {
      return d.before + ' places became ' + d.after + from + '. ' +
        d.removed + ' went, ' + d.added + ' arrived.';
    }
    return d.before + ' places became ' + d.after + from + '. ' +
      d.removed + (d.removed === 1 ? ' place went.' : ' places went.');
  }

  function undoPasteText(entry) {
    if (!entry) return '';
    var n = entry.ids.length;
    return 'Undo last paste (' + n + ' place' + (n === 1 ? '' : 's') + ')';
  }
  ...
  function intakeRead(text, opts) {
    ...
    if (s) {
      ...
      if (mode === 'delta' && looksWholeGuide(s.data)) {
        // Whether it is a PERFECT guide decides how much help we can give, not
        // whether we recognise it. The first version of this only fired on a
        // fully valid guide, so a guide with one schema slip fell through to
        // the delta errors: the traveler was told "delta must be true" about a
        // payload that was never meant to be a delta, and the real problem was
        // hidden in a check nobody ran.
        var asCity = checkPayload(s.data, 'city', opts);
        return { ok: false, data: null, route: s.route, repairs: s.repairs, tolerant: true,
          found: 'json', errors: asCity.ok ? [] : asCity.errors, convertible: false,
          wrongBox: 'city',
          message: asCity.ok
            ? 'That is a whole city guide, not a top-up, so it does not belong in this box. ' +
              'Close this and use Update data instead, which replaces the city\'s research ' +
              'with it. Everything you have already planned here would be replaced too.'
            : 'That is a whole city guide, not a top-up, so it belongs in Update data rather ' +
              'than here. It is not ready for that box yet either:\n- ' +
              asCity.errors.join('\n- ') };
      }
  ...
  // CLEARING A CREDENTIAL IS NOT AN EDIT LIKE THE OTHERS.
  //
  // Every other local change can afford to fail quietly, because the retry
  // story is "local stays newer than the server, so the next pull's reconcile
  // pushes it" and the worst case is a late sync. A cleared key is different in
  // two ways. The account still holds the old value, so the thing the traveler
  // meant to revoke is still out there. And the reason people clear it is
  // usually to change which key a run spends, so "it will sync eventually" is
  // an answer to a question nobody asked.
  //
  // landed: true when the account took the clear, false when the push failed,
  // null when there was nothing to push because this device is signed out.
  function keyClearText(landed) {
    if (landed === null || landed === undefined) {
      return 'Key cleared on this device.';
    }
    if (landed) {
      return 'Key cleared on this device and on your account.';
    }
    // Named plainly, because the recovery is not obvious and the failure is
    // invisible: nothing looks wrong until another device signs in.
    return 'Key cleared on this device, but your account still has the old one. ' +
      'It will clear the next time this device syncs. Until then, signing in ' +
      'somewhere else can bring the old key back.';
  }
  ...
  // WHICH KEY IS ABOUT TO BE SPENT, said before the button is pressed.
  //
  // Earned the hard way on 2026-09-06. A saved API key silently beats the
  // managed tier (see aiTransport: `if (o.hasKey) return 'direct'`), which is
  // the right rule, because moving somebody onto our bill without being asked
  // would be deciding their spend for them. What was wrong was that the app
  // never said so. Generate was pressed three times against a key nobody
  // remembered saving, the managed proxy was never called once, and the only
  // way anyone found out was reading the database. The transport was always
  // knowable; it was just never printed.
  //
  // ctx: {hasKey, tier, entitled, signedIn} exactly as aiTransport takes it,
  // plus `usage`, which is what my_ai_usage() returned or null. The decision is
  // made by calling aiTransport rather than re-deriving it, so this line can
  // never disagree with the request that follows it.
  //
  // Returns {show, onOurKey, text}. `show` is false only for 'none', because a
  // traveler with no transport at all is already being told why by the shells'
  // aiNoTransportText, and two refusals stacked is worse than one.
  function aiTransportLine(ctx) {
    var o = ctx || {};
    var t = aiTransport(o);
    var out = { show: false, onOurKey: false, text: '' };
    if (t === 'none') return out;
    out.show = true;
    if (t === 'direct') {
      // Named in full. "Your own key" is the sentence that would have saved
      // the hour: it is the only thing that distinguishes a run we can meter
      // from a run we cannot see at all.
      out.text = 'This runs on your own Claude key, so it is billed to your Anthropic account ' +
        'and no allowance of ours applies. Clear the key in Profile to run it on our key instead.';
      return out;
    }
    out.onOurKey = true;
    var m = aiMeter(o.usage);
    // The meter read can still be in flight, and a number nobody has yet is
    // worse than no number: say what the plan includes and leave the count to
    // the next draw, exactly as the modals already re-draw on entitlement.
    if (!m.show) {
      out.text = 'This runs on our key, included in your plan.';
      return out;
    }
    if (m.atCap) {
      out.text = 'This runs on our key, and this month\'s included AI is used up. It resets ' +
        aiResetPhrase(o.usage && o.usage.resets_at) + '.';
      return out;
    }
    var left = Math.max(0, m.guidesCap - m.guidesUsed);
    out.text = 'This runs on our key. About ' + left + ' of your ' + m.guidesCap +
      ' cities of research left this month.';
    return out;
  }
  ...
  // A fetch that never got an answer. Every browser words it differently, hence
  // the four spellings; none of them carries a status, because there was none.
  var AI_NETWORK_RE = /failed to fetch|networkerror|load failed|network request failed/i;

  function aiIsNetworkError(err) {
    return AI_NETWORK_RE.test((err && err.message) ? String(err.message) : '');
  }

  // The host a URL points at, for saying which one did not answer. Deliberately
  // string work rather than `new URL`: this runs in the shells' extracted test
  // harness as well as the browser, and a bad URL must degrade to '' rather
  // than throw inside an error handler.
  function aiHostOf(url) {
    var m = /^https?:\/\/([^\/?#]+)/i.exec(String(url || ''));
    return m ? m[1] : '';
  }

  // NAME THE HOST THAT ACTUALLY FAILED.
  //
  // This function used to blame api.anthropic.com for every network-level
  // failure, whichever transport had made the request. On the managed tier the
  // host that failed is our own edge function, and a traveler was being told to
  // check a content blocker against Anthropic and to doubt a saved key that was
  // never even read. Rob hit that message three times on 2026-09-06 and 09-08,
  // on two devices, and it sent the diagnosis the wrong way each time.
  //
  // err may carry aiHost and aiTransport, attached by the caller that made the
  // request. Without them the wording stays honest by staying general rather
  // than guessing a host.
  function aiErrorText(err) {
    var msg = (err && err.message) ? String(err.message) : '';
    if (aiIsNetworkError(err)) {
      var host = (err && err.aiHost) ? String(err.aiHost) : '';
      var t = (err && err.aiTransport) || '';
      var who = host
        ? 'never reached ' + host
        : 'never reached the server it was sent to';
      // Say whose server it was ONLY when the caller said which transport made
      // the request. An unannotated error knows neither, and guessing is the
      // exact mistake this function is being fixed for.
      var whose = t === 'proxy'
        ? 'That is OUR server, so neither your key nor your plan is the problem. '
        : t === 'direct'
        ? 'That is Anthropic, so it is not an API error and your saved key is not the problem. '
        : 'Nothing answered, so this is not an API error and a saved key is not the problem. ';
      return 'That request ' + who + ', so nothing answered it. ' + whose +
        'Almost always this is a content blocker or privacy extension, a VPN, or a network ' +
        'that blocks the host above' + (host ? ', so allowing ' + host + ' fixes it' : '') +
        '. The prompt is unaffected: use Copy prompt and run it in your own Claude, which ' +
        'needs no key and no direct connection.';
    }
  ...
  // WHICH TRANSPORT TO TRY WHEN THE FIRST ONE NEVER ANSWERED.
  //
  // Only ever after a NETWORK failure. A refusal with a status is an answer,
  // and retrying an answer on somebody else's bill would be spending money to
  // repeat a result we already have.
  //
  // ctx is what aiTransport takes. The rules that matter:
  //   - from `direct`, fall back to `proxy` only if the plan actually allows
  //     it, which is the same test aiTransport already makes;
  //   - from `proxy`, fall back to `direct` only if a key exists on this
  //     device, because there is nothing to send otherwise;
  //   - never fall back to the transport that just failed, and never twice.
  // Returns '' when there is nowhere to go, which is a dead end the copy-a-
  // prompt path already covers.
  function aiFallbackTransport(current, ctx) {
    var o = ctx || {};
    if (current === 'direct') {
      // Ask the real decision what a keyless device would get. If that is the
      // proxy, the account is entitled and on a managed tier, and our key can
      // carry this run.
      var without = { hasKey: false, tier: o.tier, entitled: o.entitled, signedIn: o.signedIn };
      return aiTransport(without) === 'proxy' ? 'proxy' : '';
    }
    if (current === 'proxy') return o.hasKey ? 'direct' : '';
    return '';
  }
  ...
  // The card whose Near panel is open, or null. Session-only like the three
  // above: "what is near this" is a question being asked right now, not a
  // planning decision worth remembering across visits.
  var nearOpenFor = null;
  ...
  // Runs spreadPlan and writes the result. Every assignment goes through
  // keyForDisplayedDate and setDay, exactly as a hand move does: after a slot
  // reorder an item's stored day-key is not always its ISO date, and a second
  // way of writing a day is how those two drift apart.
  //
  // Returns what it did so the caller can say it in words. Nothing is written
  // when there is nothing to place, so the control can be offered without
  // it ever being a no-op that looks like a failure.
  function applySpreadPlan() {
    var st = ctx.store.load();
    var data = effectiveData(ctx.base, st);
    var res = spreadPlan(data, st, { today: todayIso() });
    if (!res.assignments.length) return res;
    var byId = {};
    ...
    res.assignments.forEach(function (a) {
      var it = byId[a.id];
      if (!it) return;
      setDay(st, a.id, keyForDisplayedDate(data, st, it.section, a.day));
    });
    ctx.store.save(st);
    rerender();
    return res;
  ...
  function placePinHook() {
    if (typeof window === 'undefined' || !window) return null;
    return (window.CityOpsApp && window.CityOpsApp.placePin) ? window.CityOpsApp.placePin : null;
  }

  // "Place on map" from the card itself.
  //
  // Wording, deliberately: this app already has a pin, the one that holds a
  // card at the top of its section, and isPinned/pinBtn above own that word.
  // A control that said "Pin" here would mean the other thing to anybody who
  // has used the first one, so these say "Place on map" and "Move on map" and
  // never "pin" on their own.
  //
  // .ctl-more, behind the card's expand, for the reason written on askBtn: one
  // more always-visible pill wrapped all 26 control rows on the real Ksamil
  // guide. Discovery does not depend on this control anyway. The map's own
  // "not on the map yet" list names every unplaced place and carries the same
  // action, which is where a traveler looking at a gap actually is.
  function mapPinBtn(it) {
    var hook = placePinHook();
    if (!hook || !it || placeholderReason(it)) return null;
    var placed = !!itemPoint(it);
    var b = el('button', 'ctl icon-btn place-btn ctl-more', '📍');
    b.type = 'button';
    b.setAttribute('aria-label', placed ? 'Move this place on the map' : 'Place this on the map');
    b.title = placed
      ? 'Drag its pin to the right spot'
      : 'Not on the map. Drop a pin where it actually is';
    b.onclick = function () { hook(it.id); };
    return b;
  }

  // The Near control, and the panel it opens.
  //
  // Offered only on a PLACED card. An unplaced one gets no control at all
  // rather than a control that returns nothing, because "no results" and
  // "cannot be measured" are different answers and only one of them is the
  // traveler's to fix. Since #51 he can fix it in one tap, so the panel says
  // so when he opens Near on a card that has no pin.
  function nearBtn(it) {
    if (!it || placeholderReason(it) || !itemPoint(it)) return null;
    var open = nearOpenFor === it.id;
    var b = el('button', 'ctl near-btn' + (open ? ' near-open' : ''), open ? '📍 Near ✕' : '📍 Near');
    b.type = 'button';
    var aria = open ? 'Close what is near ' + (it.name || 'this place')
      : 'What else is within a walk of ' + (it.name || 'this place');
    b.setAttribute('aria-label', aria);
    b.setAttribute('aria-expanded', open ? 'true' : 'false');
    b.title = open ? 'Back to the whole map' : 'What else is within about a 15 minute walk';
    b.onclick = function (ev) {
      ev.stopPropagation();
      nearOpenFor = open ? null : it.id;
      rerender();
    };
    return b;
  }

  // The answer, in place, not on a new screen: he is holding this card.
  function nearPanel(it, data) {
    var panel = el('div', 'nearpanel');
    var list = nearbyOf(data, it.id);
    panel.appendChild(el('p', 'diff', nearSummaryText(list, it.name || '')));
    if (!list.length) {
      // Not a failure. On a guide where little is placed this is the common
      // answer, and it should read as "nothing nearby YET".
      panel.appendChild(el('p', 'mapwhy',
        'Places still missing from the map cannot be counted here. Anything you place ' +
        'starts showing up.'));
      return panel;
    }
    var ul = el('ul', 'nearlist');
    list.forEach(function (n) {
      var li = el('li', 'nearrow');
      var dot = el('span', 'mapdot');
      dot.style.background = n.color;
      li.appendChild(dot);
      var jump = el('button', 'linklike', n.name);
      jump.type = 'button';
      jump.setAttribute('data-goto-item', n.id);
      li.appendChild(jump);
      // Minutes first: it is the number he is deciding on. The tab is second,
      // because a dinner and an activity living on different tabs is the whole
      // reason this searches across them.
      li.appendChild(el('span', 'nearmins', ' ' + n.minutes + ' min'));
      if (n.tabLabel) li.appendChild(el('span', 'neartab', ' · ' + n.tabLabel));
      ul.appendChild(li);
    });
  ...
  function itemControlRow(it, status, withDayPicker) {
    ...
    if (status !== 'archived') {
      ...
      var place = mapPinBtn(it);
      if (place) ctl.appendChild(place);
      var near = nearBtn(it);
      if (near) ctl.appendChild(near);
  ...
  function appendCard(container, it, status, showWhen, withDayPicker, dayChip, sec) {
    ...
    // Where this one is, from the card. The map answers "where" and the card
    // answers "what"; this is the second half of that door, so a place found
    // by reading can be found again by looking. Only drawn when the place can
    // actually be pointed at.
    if (itemPoint(it)) container.setAttribute('data-mappable', '1');
    ...
    // Last of all, so the card reads title, detail, controls, then the answer
    // to the question the controls just asked.
    if (nearOpenFor === it.id && ctx) {
      container.appendChild(nearPanel(it, effectiveData(ctx.base, curState || emptyState())));
    }
  }
  ...
  // ---- drawing a map ----
  //
  // The engine owns the BLOCK: the container, the legend, the "not on the map
  // yet" list and the empty state. Leaflet itself is the shell's, because it
  // is a network dependency and this file has to keep working with no network
  // at all. If the library never arrives the block still renders, still lists
  // what it could not place, and the page around it is untouched.
  var mapSeq = 0;

  // Where to open a map that has nothing on it yet: the city's own coordinate
  // once it is known, cached on `city.geo` the same way an item's is.
  function cityCenter(data) {
    var c = data && data.city && data.city.geo;
    if (!c) return null;
    var ok = okLatLng(Number(c.lat), Number(c.lng));
    return ok ? { lat: ok.lat, lng: ok.lng } : null;
  }

  function cacheCityGeo(data, lat, lng) {
    var ok = okLatLng(Number(lat), Number(lng));
    if (!data || !data.city || !ok) return null;
    var out = deepClone(data);
    out.city.geo = { lat: ok.lat, lng: ok.lng, source: 'nominatim',
      at: new Date().toISOString().slice(0, 10) };
    return out;
  }

  // The Near answer as map options, for every map on the page. One helper so
  // the four call sites cannot drift into dimming differently.
  function nearMapOpts(data) {
    if (!nearOpenFor) return {};
    var list = nearbyOf(data, nearOpenFor);
    var ids = list.map(function (n) { return n.id; });
    ids.push(nearOpenFor);        // the anchor stays bright
    return { nearIds: ids, nearAnchor: nearOpenFor };
  }
  ...
  // How many done places this map COULD show, whether or not it is showing
  // them. Same list the map was built from, so the number on the button always
  // matches what the button would do.
  function countDone(data, items) {
    var list = Array.isArray(items) ? items : ((data && data.items) || []);
    var n = 0;
    list.forEach(function (it) {
      if (!it || it.status !== 'done') return;
      if (!itemPoint(it)) return;
      n++;
    });
    return n;
  }

  function renderMapBlock(data, items, opts) {
    var o = opts || {};
    var r = mapPoints(data, items, { hideDone: hideDoneOnMaps });
    var wrap = el('div', 'mapblock');
    if (!r.points.length && !r.missing.length) return null;

    // ALWAYS a canvas, even with nothing placed yet.
    //
    // This is the bug that hid the map from Rob for two days. The canvas used
    // to be drawn only `if (r.points.length)`, and the shell only loaded
    // Leaflet for blocks that had one. A fresh city has no cached coordinates,
    // so: no points, no canvas, no Leaflet, and the geocoder never started
    // because it was called after the loader. No pins, forever, on every city
    // anybody had not already mapped. Reproduced headless on the live app,
    // 2026-09-18: mapblock 1, canvases 0, leaflet-container 0, markers 0,
    // nominatim requests 0.
    //
    // A map with no pins yet is still a map: it shows the city, it says what
    // it is doing, and the pins arrive as they resolve.
    {
      var id = 'map-' + (++mapSeq);
      var canvas = el('div', 'mapcanvas');
      canvas.id = id;
      // Height lives here rather than in CSS so a map is never a zero-pixel
      // box on a browser that failed to load the stylesheet.
      canvas.style.height = (o.height || 260) + 'px';
      canvas.setAttribute('role', 'region');
      canvas.setAttribute('aria-label', o.label || 'Map of these places');
      wrap.appendChild(canvas);
      // What the shell needs to draw into it, without re-deriving any of it.
      // When a Near panel is open, every pin that is NOT in the answer is
      // dimmed, so the cluster the traveler asked about is visible at a glance
      // rather than having to be read off a list. The anchor stays bright.
      var nearSet = null;
      if (o.nearIds) {
        nearSet = {};
        o.nearIds.forEach(function (nid) { nearSet[nid] = 1; });
      }
      r.points.forEach(function (pt) {
        pt.dim = !!(nearSet && !nearSet[pt.id]);
        pt.anchor = !!(o.nearAnchor && pt.id === o.nearAnchor);
      });
      wrap.mapPayload = { id: id, points: r.points, fit: r.points.length > 0,
        center: cityCenter(data), missing: r.missing.length,
        dimming: !!nearSet };

      if (o.legend !== false) {
        var seen = {};
        r.points.forEach(function (p) { if (!p.done) seen[p.category] = 1; });
        var legend = el('div', 'maplegend');
        mapCategories().forEach(function (c) {
          if (!seen[c.id]) return;
          var chip = el('span', 'mapkey');
          var dot = el('span', 'mapdot');
        ...
        // The done key, and the control that removes them, together: the chip
        // explains the grey and the button acts on it, so the legend answers
        // "what is that grey one" and "I do not want to see them" in one place.
        //
        // Counted from the WHOLE list rather than from r.points, because when
        // they are hidden r.points has none of them and the way back would
        // disappear with them. That is the bug this ordering prevents: a
        // toggle you can turn on and not off.
        var doneCount = countDone(data, items);
        if (doneCount) {
          var doneChip = el('span', 'mapkey');
          var doneDot = el('span', 'mapdot');
          doneDot.style.background = DONE_COLOR;
          doneChip.appendChild(doneDot);
          doneChip.appendChild(el('span', null, 'Done'));
          legend.appendChild(doneChip);

          var t = el('button', 'mapkey linklike hide-done',
            hideDoneOnMaps ? 'Show done (' + doneCount + ')' : 'Hide done (' + doneCount + ')');
          t.type = 'button';
          t.setAttribute('aria-pressed', hideDoneOnMaps ? 'true' : 'false');
          t.title = hideDoneOnMaps
            ? 'Put the places you have already done back on every map'
            : 'Take the places you have already done off every map';
          t.onclick = function (ev) {
            ev.stopPropagation();
            setHideDone(!hideDoneOnMaps);
            rerender();
          };
    ...
    if (r.missing.length) {
      var det = el('details', 'mapmissing');
      var sum = el('summary', null, r.missing.length +
        (r.missing.length === 1 ? ' place is not on the map yet' : ' places are not on the map yet'));
      det.appendChild(sum);
      var ul = el('ul', 'mapmissing-list');
      r.missing.forEach(function (m) {
        var li = el('li');
        var b = el('button', 'linklike', m.name);
        b.type = 'button';
        b.setAttribute('data-goto-item', m.id);
        li.appendChild(b);
        li.appendChild(el('span', 'mapwhy', ' ' + (m.why || '')));
        // The action, next to the gap it fixes. Offered for anything real:
        // "not looked up yet" resolves itself in a second, but the 18 Istanbul
        // places OpenStreetMap has never heard of will sit in this list
        // forever, and this button is the only thing that ever moves them out
        // of it. Not offered for placeholder text, which is not a place.
        if (!m.placeholder) {
          var pb = el('button', 'linklike place-it', 'Place it');
          pb.type = 'button';
          pb.setAttribute('data-place-item', m.id);
  ...
  function renderTabSections(vms, state, totalItems) {
    ...
    var tabMap = tabItems.length
      ? renderMapBlock(effectiveData(ctx.base, state), tabItems,
          mergeOpts({ height: 240, label: 'Map of these places' },
            nearMapOpts(effectiveData(ctx.base, state))))
      : null;
    if (tabMap) main.appendChild(tabMap);
  ...
  function renderInfoTab(vms, state, totalItems) {
    ...
    var infoMap = infoItems.length
      ? renderMapBlock(effectiveData(ctx.base, state), infoItems,
          mergeOpts({ height: 220, label: 'Map of these places' },
            nearMapOpts(effectiveData(ctx.base, state))))
      : null;
  ...
  function planDayList(iso, entries, data) {
    ...
    // A day's own map, same colours, shorter: on the day you are living, the
    // question is how far apart today's places are, not where the week is.
    if (data) {
      var dayMap = renderMapBlock(data, entries.map(function (e) { return e.it; }),
        mergeOpts({ height: 180, legend: false, label: 'Map of this day' }, nearMapOpts(data)));
      if (dayMap) list.appendChild(dayMap);
    }
    entries.forEach(function (e) {
  ...
  function renderPlanTab(data, state) {
    ...
    // One map over the whole stay, before anything else on the page: where the
    // places ARE is the first question somebody standing in the city asks.
    var planMap = renderMapBlock(data, null,
      mergeOpts({ height: 300, label: 'Map of every place in this guide' }, nearMapOpts(data)));
    if (planMap) main.appendChild(planMap);
    ...
    todayGroup.appendChild(planDayList(pm.todayIso, pm.today, data));
    ...
    pm.days.forEach(function (d) {
      ...
      if (!collapsed) group.appendChild(planDayList(d.iso, d.items, data));
  ...
  function rerender() {
    ...
    revealJustMoved();
    restoreGripFocus();
    // Additive hook: the shell draws Leaflet into every .mapblock #main now
    // holds. Kept out of the engine on purpose, because a tile library is a
    // network dependency and this file has to render with no network at all.
    if (CityOps.onMaps) { try { CityOps.onMaps(); } catch (e) { /* a map is never worth a blank page */ } }
  ...
  return {
    ...
    promptKit: {
      ...
      // Staged generation: a whole city as a few short passes rather than one
      // call that outlives the edge function's wall clock.
      ...
      // An in-app run that stops is OUR failure, and says so.
      runFragmentText: runFragmentText, aiRunTimeoutText: aiRunTimeoutText,
      // Taking a paste back, and spotting template text that should never
      // have landed in a real guide.
      placeholderItems: placeholderItems, placeholderText: placeholderText,
      placeholderReason: placeholderReason,
      removeItemsById: removeItemsById, recordPaste: recordPaste,
      lastPaste: lastPaste, undoPasteText: undoPasteText,
      addSnapshot: addSnapshot, newestSnapshot: newestSnapshot,
      replaceDamage: replaceDamage, isBigReplace: isBigReplace,
      replaceUndoText: replaceUndoText, SNAPSHOT_MAX: SNAPSHOT_MAX,
      aiRunProgressText: aiRunProgressText, AI_RUN_TIMEOUT_MS: AI_RUN_TIMEOUT_MS,
    ...
    aiProxyKit: {
      ...
      // Told apart so the shells can retry a request that never got an answer,
      // and name the host that did not answer it.
      isNetworkError: aiIsNetworkError, hostOf: aiHostOf,
      fallbackTransport: aiFallbackTransport,
    ...
    syncKit: {
      ...
      // What Profile says after Clear key, which depends on whether the ACCOUNT
      // took the clear and not just this device.
      keyClearText: keyClearText,
    ...
    safeHref: safeHref,
    onStateChange: null, onMaps: null, liveState: liveState,
    ...
    // Maps. The parser and the colour mapping are pure and tested; the shell
    // owns Leaflet, the tiles and the one-per-second geocoding.
    mapKit: {
      categories: mapCategories, category: mapCategory, color: mapColor,
      parseLatLng: parseLatLng, parseMapQuery: parseMapQuery, mapLink: itemMapLink,
      point: itemPoint, geocodeQuery: geocodeQuery, points: mapPoints, cacheGeo: cacheGeo,
      cityCenter: cityCenter, cacheCityGeo: cacheCityGeo,
      plan: geocodePlan, accept: acceptGeocode, district: districtFrom,
      placeByHand: placeByHand, clearPoint: clearPoint, isHandPlaced: isHandPlaced,
      haversineM: haversineM, walkMinutes: walkMinutes, nearby: nearbyOf,
      hideDone: hideDoneNow, setHideDone: setHideDone, DONE_COLOR: DONE_COLOR,
      nearSummary: nearSummaryText, nearUnavailable: nearUnavailableText,
      NEAR_RADIUS_M: NEAR_RADIUS_M,
      shortName: shortName,
      BBOX_PAD_DEG: BBOX_PAD_DEG,
      renderBlock: renderMapBlock
    },
    ...
    dayMoveOptions: dayMoveOptions, spreadPlan: spreadPlan, applySpreadPlan: applySpreadPlan,
