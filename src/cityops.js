var CityOps = (function () {
  ...
  // Per stage, and deliberately UNDER ai_reserve's large-call threshold of
  // 8000. That is not a way around the meter: the monthly cap counts real
  // output tokens whichever route they arrive by, so the money is identical.
  // The hourly LARGE-call limit exists to stop somebody running many whole
  // guides an hour, and three short stages is still one guide, so charging
  // them against the small-call budget is the truer reading of the same rule.
  var STAGE_MAX_TOKENS = 7000;
