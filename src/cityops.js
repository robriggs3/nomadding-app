var CityOps = (function () {
  ...
  function buildResearchAllPrompt(templateText, cityData, profile) {
    ...
    out.push(coverageBlock(cityData, null, null), '');
    out.push('## Existing items (do not re-suggest these)', '');
    out = out.concat(itemListLines(cityData && cityData.items));
    out.push('');
    out.push('## Item shape', '', contract, '');
  ...
  // What one item costs to write, near enough to budget a stage with. Measured
  // from the 2026-09-14 runs: three stages returned 6,315, 6,645 and 6,155
  // output tokens for 16, 17 and 16 items, so a little under 400 each. 350 is
  // the working figure with the margin left in on purpose.
  var TOKENS_PER_ITEM = 350;
